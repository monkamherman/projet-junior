...
const signupSchema = zod_1.z.object({
    nom: zod_1.z
        ...
        .regex(/^[a-zA-ZÀ-ÿ\-\s]+$/, "Le nom ne doit contenir que des lettres et des espaces"),
    prenom: zod_1.z
        .string()
        .min(2, "Le prénom doit contenir au moins 2 caractères")
        .max(50, "Le prénom ne peut pas dépasser 50 caractères")
        .regex(/^[a-zA-ZÀ-ÿ\-\s]+$/, "Le prénom ne doit contenir que des lettres et des espaces"),
    email: zod_1.z
        .string()
    ...
    telephone: zod_1.z
        .string()
        .regex(/^[0-9]{10,15}$/, "Numéro de téléphone invalide. Format attendu : 0612345678")
        .optional(),
    otp: zod_1.z.string().length(6, "Le code OTP doit contenir 6 chiffres").optional(),
});
...
async function signup(req, res) {
    try {
        ...
        if (existingUser) {
            console.log(`Tentative de création d'un compte avec un email existant: ${email}`);
...
async function sendOTP(req, res) {
    try {
        ...
        if (existingUser) {
            console.log(`Tentative d'envoi d'OTP pour un email déjà enregistré: ${email}`);
            return res.status(409).json({
                success: false,
                message: "Un compte existe déjà avec cette adresse email.",
                code: "EMAIL_ALREADY_EXISTS",
...
L'équipe Centic`;
        ...
        const sendResult = await (0, sendmail_1.default)(email, emailContent);
